...
def page_radar(): 
    ...
    with radar_col1:
        ...
        # Aplicar filtros reutilizables para Player A
        ...
        # Crear diccionario label -> nombre real
        label_to_player_A = dict(zip(players_info_A["label"], players_info_A["Player"]))

        # Obtener valor actual guardado
        current_playerA = st.session_state.get("playerA", "Select")

        # Crear opciones del selector
        playerA_options = ["Select"] + list(label_to_player_A.keys())

        # Resetear jugador si ya no está disponible
        if current_playerA not in playerA_options:
            st.session_state["playerA"] = "Select"
            current_playerA = "Select"

        # Selector de jugador
        playerA_label = st.selectbox(
            "Player A",
            playerA_options,
            index=playerA_options.index(current_playerA),
            key="playerA"
        )

        # Convertir label en nombre real
        playerA = label_to_player_A[playerA_label] if playerA_label != "Select" else None

    with radar_col3:
        st.subheader("Player B")

        # Aplicar filtros reutilizables para Player B
        ...
        # Obtener valor actual guardado
        current_playerB = st.session_state.get("playerB", "Select")

        # Crear opciones del selector
        playerB_options = ["Select"] + list(label_to_player_B.keys())

        # Resetear jugador si ya no está disponible
        if current_playerB not in playerB_options:
            st.session_state["playerB"] = "Select"
            current_playerB = "Select"

        # Selector de jugador
        playerB_label = st.selectbox(
            "Player B",
            playerB_options,
            index=playerB_options.index(current_playerB),
            key="playerB"
        )
