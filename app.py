...
def login_screen():
    ...
    # Mostrar logo según dispositivo
    if not is_mobile:
    ...
    with col2:
        ...
        # Mensaje de acceso visitante
        st.markdown(
            "<p style='text-align:center; font-size:15px; color:#e2e8f0;'>"
            "* To access as a Visitor enter 'guest' in User and Password"
            "</p>",
        ...
        if st.button("Access"):
            ...
            if user.strip() == "guest" and password.strip() == "guest":
                ...
                # Registrar acceso
                log_access("Guest", "viewer")

            else:
                # Autenticar usuario en la base de datos
                match = authenticate_user(user.strip(), password)
                ...
                if match:
                    ...
                    # Registrar acceso
                    log_access(match.username, match.role)
